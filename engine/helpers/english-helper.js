global.EnglishHelper = {
  ...
  roundWidthComparator(width) {
    ...
    if (width < 52)  { return Random.from(['avocado','lemon', 'lime', 'tomato']); }
  ...
  pluralRoundWidthComparator(width) {
    ...
    if (width < 52)  { return Random.from(['avocados', 'lemons', 'limes', 'tomatoes']); }
    if (width < 64)  { return Random.from(['plums', 'tomatoes','peaches']); }
    if (width < 76)  { return Random.from(['baseballs', 'fists', 'tomatoes']); }
    if (width < 89)  { return Random.from(['peaches', 'lemons', 'fists', 'doorknobs']); }
    if (width < 102) { return Random.from(['softballs','apples','fists', 'potatoes']); }
    if (width < 127) { return Random.from(['oranges', 'apples', 'onions']); }
    if (width < 152) { return Random.from(['onions', 'apples', 'bell peppers']); }
    if (width < 178) { return Random.from(['mangoes', 'onions', 'grapefruits', 'sweet potatoes','ostrich eggs']); }
    if (width < 203) { return Random.from(['grapefruits', 'footballs', 'sweet potatoes']); }
    ...
    if (width < 305) { return Random.from(['cantaloupes', 'melons', 'coconuts', 'squashes', 'milk jugs']); }
